...
const SUMMER_PERIODS = { 2026: ['2026-07-27','2026-08-21'] };
...
function timeColumns(rawTable,stopCount){
  ...
  for(let c=0;c<cols;c++){
    let rowsWithTime=0;
    for(const row of rawTable.data||[]) if(Array.isArray(row) && normalizeTime(row[c])) rowsWithTime++;
...
function buildGroup(lineId,pageNumber,summaryTable,rawTable){
  ...
  return {lineId,page:pageNumber,table:summaryTable.index,stops,trips,...classifyGroup(pageNumber,summaryTable)};
}
...
function groupsForDate(serviceDate,lineValue='all'){
  ...
  let groups=source.filter(g=>g.dayType===type && g.season===season);
...
function buildConnections(serviceDate,lineValue,originName,minDeparture,maxArrival,transfer,destinationId){
  ...
  return [...byE13.values()].sort((a,b)=>a.e13.arr-b.e13.arr || b.urban.dep-a.urban.dep).slice(0,6);
}
...
async function ensureData(){
  ...
  if(!loadPromise){
    ...
    loadPromise=(async()=>{
      ...
      if(!summaryRes.ok || !tableRes.ok) throw new Error('No se han podido descargar los horarios urbanos');
      ...
      if(allGroups.length) searchBtn.disabled=!stopSelect.value;
    });
  }
  return loadPromise;
}
async function searchRoutes(){
  ...
  try{
    ...
    if(!routes.length){
      resultsBox.className='planner-results empty-state';
      resultsBox.innerHTML=`No encuentro una combinación <strong>${stopSelect.value}</strong> → Rodalies → e13 dentro de ese margen. Prueba una salida anterior, una llegada más tarde o un margen de transbordo menor.`;
      return;
    }
    ...
    resultsBox.className='planner-results empty-state';
    resultsBox.innerHTML='El planificador no ha podido cargar los datos. Recarga la página y vuelve a intentarlo. Las vistas e13 y Mataró Bus siguen funcionando con normalidad.';
  }
}
...
lineSelect.addEventListener('change',async()=>{ try{await ensureData();refreshStops();await searchRoutes();}catch{} });
dateInput.addEventListener('change',async()=>{ try{await ensureData();refreshStops();await searchRoutes();}catch{} });
stopSelect.addEventListener('change',()=>void searchRoutes());
departInput.addEventListener('change',()=>void searchRoutes());
arriveInput.addEventListener('change',()=>void searchRoutes());
transferSelect.addEventListener('change',()=>void searchRoutes());
destinationSelect.addEventListener('change',()=>void searchRoutes());
searchBtn.addEventListener('click',()=>void searchRoutes());
...
ensureData().then(()=>searchRoutes()).catch(()=>{
  statusBox.textContent='No se han podido cargar los horarios urbanos.';
  resultsBox.className='planner-results empty-state';
  resultsBox.textContent='El planificador no está disponible ahora mismo. Pulsa Buscar para reintentar tras recargar la página.';
});
