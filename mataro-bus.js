...
const globalCalendarInfo=document.getElementById('globalCalendarInfo');
...
root.innerHTML=lines.map(([id,name])=>`<button class="urban-line" type="button" data-line="${id}"><span class="line-badge">${id}</span><span style="min-width:0"><strong>Línea ${id}</strong><span>${name}</span></span><span style="margin-left:auto;font-size:20px;opacity:.45">›</span></button>`).join('');
...
function extractGroups(summaryLine,rawLine){
  const out=[];
  for(const page of summaryLine||[])for(const table of page.tables||[]){
    const rawPage=(rawLine||[]).find(p=>p.page===page.page);const rawTable=rawPage?.tables?.find(t=>t.index===table.index);
    if(!rawTable?.data?.length||!table.pairs?.length)continue;
    const cols=rawTable.cols||Math.max(...rawTable.data.map(r=>r.length));const timeCols=[];
    for(let c=0;c<cols;c++){const times=[];for(const row of rawTable.data)times.push(...findTimes(row[c]));const unique=[...new Set(times)].sort((a,b)=>minutes(a)-minutes(b));if(unique.length>=2)timeCols.push(unique)}
    const stops=table.pairs.map((p,i)=>({name:p.name||p.raw_name||`Parada ${i+1}`,times:timeCols[i]||[]})).filter(s=>s.times.length);
    if(stops.length)out.push({page:page.page,table:table.index,stops,...classifyGroup(page.page,table)});
  }
  return out;
}
...
function applyCalendar(){
  ...
  const day=selectedDayType(),season=selectedSeason();
  visibleGroups=groups.filter(g=>g.dayType===day&&g.season===season);
  if(!visibleGroups.length)visibleGroups=groups.filter(g=>g.dayType===day);
  if(!visibleGroups.length)visibleGroups=groups.slice(0,1);
  scheduleSelect.innerHTML=visibleGroups.map((g,i)=>`<option value="${i}">${directionLabel(g,i)}</option>`).join('');
  renderStops();
}
...
async function openLine(id){
  currentLine=id;const line=lines.find(([n])=>n===id);detailTitle.textContent=`Línea ${id} · ${line?.[1]||''}`;pdfLink.href=`./mataro-bus-official/line-${id}.pdf`;detail.classList.remove('hidden');nextDepartures.innerHTML='<div class="next-box">Cargando horario…</div>';allTimes.innerHTML='';detail.scrollIntoView({behavior:'smooth',block:'start'});
  try{
    if(!summaryData||!tableData){const [summaryRes,tableRes]=await Promise.all([fetch('./mataro-bus-table-summary.json',{cache:'no-store'}),fetch('./mataro-bus-tables.json',{cache:'no-store'})]);if(!summaryRes.ok||!tableRes.ok)throw new Error();[summaryData,tableData]=await Promise.all([summaryRes.json(),tableRes.json()])}
    groups=extractGroups(summaryData[String(id)],tableData[String(id)]);if(!groups.length)throw new Error();applyCalendar();
  }catch(e){scheduleSelect.innerHTML='';stopSelect.innerHTML='';nextDepartures.innerHTML='<div class="next-box"><strong>No he podido mostrar los datos.</strong><div style="margin-top:6px">Puedes abrir el PDF oficial debajo.</div></div>'}
}

root.addEventListener('click',e=>{const b=e.target.closest('[data-line]');if(b)openLine(Number(b.dataset.line))});
globalCalendar.addEventListener('change',applyCalendar);
